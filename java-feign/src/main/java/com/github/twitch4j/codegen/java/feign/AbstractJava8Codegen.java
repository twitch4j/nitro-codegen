...
@Slf4j
public abstract class AbstractJava8Codegen extends DefaultCodegen implements CodegenConfig {
    ...
    public void processOpts() {
        ...
        if (StringUtils.isEmpty(System.getenv("JAVA_POST_PROCESS_FILE"))) {
            log.info("Environment variable JAVA_POST_PROCESS_FILE not defined so the Java code may not be properly formatted. To define it, try 'export JAVA_POST_PROCESS_FILE=\"/usr/local/bin/clang-format -i\"' (Linux/Mac)");
            log.info("NOTE: To enable file post-processing, 'enablePostProcessFile' must be set to `true` (--enable-post-process-file for CLI).");
        }
        ...
        if (additionalProperties.containsKey(CodegenConstants.INVOKER_PACKAGE)) {
            ...
            this.setInvokerPackage((String) additionalProperties.get(CodegenConstants.INVOKER_PACKAGE));
            log.info("Invoker Package Name, originally not set, is now derived from api package name: {}", derivedInvokerPackage);
            ...
            this.additionalProperties.put(CodegenConstants.INVOKER_PACKAGE, derivedInvokerPackage);
            this.setInvokerPackage((String) additionalProperties.get(CodegenConstants.INVOKER_PACKAGE));
            log.info("Invoker Package Name, originally not set, is now derived from model package name: {}", derivedInvokerPackage);
        } else {
    ...
    public void setParameterExampleValue(CodegenParameter codegenParameter, RequestBody requestBody) {
        ...
        if (content.size() > 1) {
            ...
            log.warn("Multiple MediaTypes found, using only the first one");
        }

        MediaType mediaType = content.values().iterator().next();
        if (mediaType.getExample() != null) {
            if (isModel) {
                log.warn("Ignoring complex example on request body");
            } else {
    ...
    public String toOperationId(String operationId) {
        ...
        if (operationId.matches("^\\d.*")) {
            log.warn(operationId + " (starting with a number) cannot be used as method name. Renamed to " + camelize("call_" + operationId), true);
    ...
    public CodegenModel fromModel(String name, Schema model) {
        ...
        if (allDefinitions != null && codegenModel.parentSchema != null && codegenModel.hasEnums) {
            ...
            codegenModel = AbstractJava8Codegen.reconcileInlineEnums(codegenModel, parentCodegenModel);
        }
    ...
    public void preprocessOpenAPI(OpenAPI openAPI) {
        ...
        if (additionalProperties.containsKey(CodegenConstants.SNAPSHOT_VERSION)) {
            if (convertPropertyToBooleanAndWriteBack(CodegenConstants.SNAPSHOT_VERSION)) {
                this.setArtifactVersion(this.getArtifactVersion());
            }
        }
        additionalProperties.put(CodegenConstants.ARTIFACT_VERSION, artifactVersion);
    }
    ...
    public void postProcessFile(File file, String fileType) {
        ...
        if ("java".equals(FilenameUtils.getExtension(file.toString()))) {
            ...
            try {
                ...
                if (exitValue != 0) {
                    log.error("Error running the command ({}). Exit value: {}", command, exitValue);
                } else {
